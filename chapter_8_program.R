# Sentiment Analysis Using the Movie Ratings Data
# confusion matrix : http://www.inside-r.org/node/86995
# termFreq : http://www.inside-r.org/packages/cran/tm/docs/termFreq
# svm : http://www.inside-r.org/node/57517
...
positive.data.frame <- read.table(file = "C:/Users/Administrator/Desktop/DataScience/CODE/R/Chapter_8/Hu_Liu_positive_word_list.txt",
...
negative.data.frame <- read.table(file = "C:/Users/Administrator/Desktop/DataScience/CODE/R/Chapter_8/Hu_Liu_negative_word_list.txt",
                                  header = FALSE, colClasses = c("character"), row.names = NULL, 
...
Hu.Liu.positive.dictionary <- positive.data.frame$positive.words
...
# across the document collection... as it turns out this is 25 words
# a review of this list suggests that all make sense (have content validity)
test.positive.dictionary <- Hu.Liu.frequent.positive

# .... now for the negative words
Hu.Liu.negative.dictionary <- negative.data.frame$negative.words
...
top.words <- Terms(examine.tdm)
print(top.words)    
Hu.Liu.frequent.negative <- findFreqTerms(reviews.tdm.Hu.Liu.negative, 16)  
...
test.negative.dictionary <- test.negative # set of 32 words
...
with(text.measures.data.frame, print(cor(POSITIVE, NEGATIVE)))  # -0.09464118 
...
# compute list-based text measures for the test corpus
# for each of the documents count the total words 
# and the number of words that match the positive and negative dictionaries
...
tom.movies <- data.frame(movies = 
                           c("The Effect of Gamma Rays on Man-in-the-Moon Marigolds",
                             ...
                             "Fight Club","Miss Congeneality 2","Find Me Guilty","Moneyball"))
...
with(train.data.frame, print(cor(simple, rating)))  # positive r = 0.4252565
...
print(try.tree)  # note that the first split is at -0.7969266
# create a user-defined function for the simple difference method
predict.simple <- function(x) {
  if (x >= 1.886792) return("UP")
  if (x < 1.886792) return("DOWN")
}
...
predict.regression <- function(x) {
  if (x >= 5.437744) return("UP")
  if (x < 5.437744) return("DOWN")
}
...
# report full set of statistics relating to predictive accuracy
print(train.pred.regression.performance)  # result 67.6 Percent
...
# report full set of statistics relating to predictive accuracy
print(test.pred.regression.performance)  # result 66.3 Percent
...
phi <- numeric(62)
item <- c(test.positive.dictionary,test.negative.dictionary)
item.analysis.data.frame <- data.frame(item,phi)
item.place <- 14:63
for (index.for.column in 1:50) {
  item.analysis.data.frame$phi[index.for.column] <- 
    cor(train.data.frame[, item.place[index.for.column]],train.data.frame[,8])  # correlation words columns with rating column. 
}
...
selected.items.data.frame <- 
  subset(item.analysis.data.frame, absphi > 0.05)

# use the sign of the phi coefficient as the item weight
selected.positive.data.frame <-
  subset(selected.items.data.frame, phi > 0.0)
...
# try alternative positive and negative word sets
# try word sets that relate to a wider variety of emotional or opinion states
# better still... move beyond a simple bag-of-words approach to sentiment
# run a true benchmark within a loop using hundreds or thousands of iterations
# try other indices of classifier performance such as area under the ROC curve
